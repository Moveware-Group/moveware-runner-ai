...
ERROR_PATTERNS = {
    ...
    "type_error": {
        ...
        "fix_hint": (
            "**TYPESCRIPT TYPE ERROR — SYSTEMATIC FIX REQUIRED:**\n\n"
            "**Step 1: Identify the exact mismatch**\n"
            "- What type do you have? (e.g. TenantCredentials)\n"
            "- What type is expected? (e.g. MovewareRequestContext)\n"
            "- What properties are missing?\n\n"
            "**Step 2: Find the correct pattern in existing code**\n"
            "- Search the codebase for how other files use the expected type\n"
            "- Look at how the expected type is constructed/imported\n"
            "- Copy the exact pattern, don't invent new approaches\n\n"
            "**Step 3: Apply the right fix**\n"
            "- If missing properties: construct the object with all required fields\n"
            "- If interface mismatch: find how existing code converts between the types\n"
            "- If array element type: check the function signature for what array type it expects\n\n"
            "**COMPONENT PROPS ERRORS (most common):**\n"
            "When the error says \"Property 'X' does not exist on type 'SomeComponentProps'\":\n"
            "1. **READ the component file** — it is provided in the context below!\n"
            "2. Find the Props interface/type — see EXACTLY what props it accepts\n"
            "3. You have TWO options:\n"
            "   a) **Add the prop to the interface** — if this is new data the component needs,\n"
            "      add it to the Props type AND add logic in the component to use it\n"
            "   b) **Remove the prop from the caller** — if the component doesn't need it,\n"
            "      remove it from where it's passed in page.tsx\n"
            "4. **BOTH files must be consistent** — the page.tsx props must match the component's Props type\n"
            "5. **Include BOTH files in your fix** — update the component AND the page that uses it\n\n"
            "**CRITICAL: Do NOT use 'as any' casts to silence type errors.** "
            "Find and use the correct type. Check existing code for the exact pattern.\n\n"
            "**COMMON TYPE CONVERSIONS:**\n"
            "- number → string: String(value) or `${value}`\n"
            "- string → number: Number(value) or parseInt(value)\n"
            "- undefined → required: Add null check or ?? default\n"
            "- Record<string, unknown>: Cast with `value as Record<string, unknown>`"
        )
    },
    "react_hook_error": {
        "patterns": [
