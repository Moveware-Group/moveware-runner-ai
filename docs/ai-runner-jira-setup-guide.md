...
| **AI Console shows completed but Jira still In Progress** | The run finished but Jira wasn't transitioned. **1. Check status names:** `JIRA_STATUS_IN_TESTING` in .env must match Jira exactly (e.g. "In Testing"). **2. Check worker logs** for `Warning: No transition to '...'` – lists available Jira transitions. **3. Quick fix:** Manually move the issue to In Testing in Jira (the code is already committed). |
