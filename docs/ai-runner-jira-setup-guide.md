...
| **Run does not fire** | Jira shows SUCCESS but worker never processes. **1. Orchestrator logs:** After deploy, check `journalctl -u moveware-ai-orchestrator -f` – you should see `[webhook] TB-2 -> run_id=123` when the rule fires. If not, the request isn't reaching the app (nginx, URL, or rule conditions). **2. Queue stats:** `curl .../api/queue/stats` – if `total_queued` > 0 but worker never claims, try `POST /api/queue/reset-stale`. **3. Manual trigger:** `POST /api/trigger` with `{"issue_key":"TB-2"}` and `X-Admin-Secret` to force a run. **4. Assignee:** Story must be assigned to AI Runner; router ignores otherwise. |
